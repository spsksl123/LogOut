...
void logMessage(char* buffer, size_t *currentLengh, size_t bufferSize, const char* format, ...) {
    ...
    if (*currentLengh < 200) {
            *currentLengh += vsnprintf(&buffer[*currentLengh], bufferSize - *currentLengh, format, args);
    } else {
          *currentLengh += vsnprintf(&buffer[*currentLengh], bufferSize - *currentLengh, format, args);
...
void exampleUsage() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        logMessage(logSave1, &currentLengh, sizeof(logSave1), "[%d] r:%d a:%d np:%.1f ", detIdx, rangeIndx, angleIndx, noise);
